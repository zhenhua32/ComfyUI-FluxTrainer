...
class TrainDatasetGeneralConfig:
        ...
        return {
            "required": {
                ...
                "caption_dropout_rate": (
                    "FLOAT",
                    {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "tag dropout rate"},
                ),
            ...
            "optional": {
                "reset_on_queue": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "Force refresh of everything for cleaner queueing"},
                ),
    ...
    def create_config(
        ...
        # print(dataset_json)
        dataset_config = {"datasets": dataset_json, "alpha_mask": alpha_mask}
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "num_repeats": (
                    "INT",
                    {"default": 1, "min": 1, "tooltip": "number of times to repeat dataset for an epoch"},
                ),
            },
        }
...
class TrainDatasetAdd:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "enable_bucket": (
                    "BOOLEAN",
                    {"default": True, "tooltip": "enable buckets for multi aspect ratio training"},
                ),
                ...
                "min_bucket_reso": (
                    "INT",
                    {"default": 256, "min": 64, "max": 4096, "step": 8, "tooltip": "min bucket resolution"},
                ),
                "max_bucket_reso": (
                    "INT",
                    {"default": 1024, "min": 64, "max": 4096, "step": 8, "tooltip": "max bucket resolution"},
                ),
            },
            "optional": {
    ...
    def create_config(
        ...
        new_dataset = {
            ...
            "subsets": [{"image_dir": dataset_path, "class_tokens": class_tokens, "num_repeats": num_repeats}],
        }
        ...
        if self.previous_dataset_signature:
            existing_datasets["datasets"] = [
                ds
                for ds in existing_datasets["datasets"]
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "lr_scheduler": (
                    ["constant", "cosine", "cosine_with_restarts", "polynomial", "constant_with_warmup"],
                ...
                "lr_scheduler_num_cycles": (
                    "INT",
                    {"default": 1, "min": 1, "tooltip": "learning rate scheduler num cycles"},
                ),
                "lr_scheduler_power": (
                    "FLOAT",
                    {"default": 1.0, "min": 0.0, "tooltip": "learning rate scheduler power"},
                ),
                "min_snr_gamma": (
                    "FLOAT",
                    {
                ...
                "extra_optimizer_args": (
                    "STRING",
                    {"multiline": True, "default": "", "tooltip": "additional optimizer args"},
                ),
            },
        }
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "lr_scheduler": (
                    ["constant", "cosine", "cosine_with_restarts", "polynomial", "constant_with_warmup", "adafactor"],
                    {"default": "constant_with_warmup", "tooltip": "learning rate scheduler"},
                ),
                "lr_warmup_steps": ("INT", {"default": 0, "min": 0, "tooltip": "learning rate warmup steps"}),
                "lr_scheduler_num_cycles": (
                    "INT",
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "network_alpha": (
                    "FLOAT",
                    {"default": 1.0, "min": 0.0, "max": 2048.0, "step": 0.01, "tooltip": "network alpha"},
                ),
                "learning_rate": (
                    "FLOAT",
            ...
            "optional": {
                "additional_args": (
                    "STRING",
                    {"multiline": True, "default": "", "tooltip": "additional args to pass to the training command"},
                ),
                ...
                "clip_l_lr": (
                    "FLOAT",
                    {"default": 0, "min": 0.0, "max": 10.0, "step": 0.000001, "tooltip": "text encoder learning rate"},
                ),
                "T5_lr": (
                    "FLOAT",
                    {"default": 0, "min": 0.0, "max": 10.0, "step": 0.000001, "tooltip": "text encoder learning rate"},
...
class InitFluxTraining:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "learning_rate": (
                    "FLOAT",
                    {"default": 4e-6, "min": 0.0, "max": 10.0, "step": 0.000001, "tooltip": "learning rate"},
                ),
                "max_train_steps": (
                    "INT",
                    {"default": 1500, "min": 1, "max": 100000, "step": 1, "tooltip": "max number of training steps"},
                ...
                "t5xxl_max_token_length": (
                    "INT",
                    {"default": 512, "min": 64, "max": 4096, "step": 8, "tooltip": "dev uses 512, schnell 256"},
                ),
                "cache_latents": (["disk", "memory", "disabled"], {"tooltip": "caches text encoder outputs"}),
                "cache_text_encoder_outputs": (
                    ["disk", "memory", "disabled"],
                ...
                "cpu_offload_checkpointing": (
                    ...
                    {
                        "default": True,
                        "tooltip": "offload the gradient checkpointing to CPU. This reduces VRAM usage for about 2GB",
                    },
                ),
                "optimizer_fusing": (
                    ["fused_backward_pass", "blockwise_fused_optimizers"],
                    {"tooltip": "reduces memory use"},
                ),
                "single_blocks_to_swap": (
                    "INT",
                    {
                        ...
                        "max": 100,
                        "step": 1,
                        "tooltip": "number of single blocks to swap. The default is 0. This option must be combined with blockwise_fused_optimizers",
                    },
                ),
                "double_blocks_to_swap": (
                    "INT",
                    {
                        ...
                        "max": 100,
                        "step": 1,
                        "tooltip": "number of double blocks to swap. This option must be combined with blockwise_fused_optimizers",
                    },
                ),
                "guidance_scale": (
                    "FLOAT",
                    {"default": 1.0, "min": 1.0, "max": 32.0, "step": 0.01, "tooltip": "guidance scale"},
                ),
                "discrete_flow_shift": (
                    "FLOAT",
                    {
                ...
                "gradient_dtype": (
                    ["fp32", "fp16", "bf16"],
                    {"default": "bf16", "tooltip": "to use the full fp16/bf16 training"},
                ),
                "save_dtype": (
                    ["fp32", "fp16", "bf16", "fp8_e4m3fn"],
                    {"default": "bf16", "tooltip": "the dtype to save checkpoints as"},
                ),
                "attention_mode": (
                    ["sdpa", "xformers", "disabled"],
    ...
    def init_training(
        ...
        **kwargs,
        ...
        if free <= required_free_space:
            raise ValueError(
                f"Most likely insufficient disk space to complete training. Required: {required_free_space/2**30}GB. Available: {free/2**30}GB"
            )
            ...
            "required": {
                ...
                "output_dir": (
                    "STRING",
                    {
                        "default": "flux_trainer_output",
                        "multiline": False,
                        "tooltip": "output directory, root is ComfyUI folder",
                    },
                ),
                "sample_prompts": (
                    "STRING",
        ...
        return {
            "required": {
                ...
                "copy_to_comfy_lora_folder": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "copy the lora model to the comfy lora folder"},
                ),
            },
        }
        ...
        with torch.inference_mode(False):
            ...
            if remove_step_no is not None:
                remove_ckpt_name = train_util.get_step_ckpt_name(
                    trainer.args, "." + trainer.args.save_model_as, remove_step_no
                )
...
class FluxTrainResume:
    ...
    def resume(self, load_state_path, skip_until_initial_step):
        resume_args = {"resume": load_state_path, "skip_until_initial_step": skip_until_initial_step}
        ...
        return {
            "required": {
                "include": (
                    "STRING",
                    {
                        "default": "lora_unet_single_blocks_20_linear2",
                        "multiline": True,
                        "tooltip": "blocks to include in the LoRA network, to select multiple blocks either input them as ",
                    },
                ),
            },
        }
            ...
            if match:
                ...
                if matches:
                    for start, end in matches:
                        ...
                        blocks.extend(
                            [
                                f"{prefix}{i}{suffix.replace(f'({start}-{end})', '', 1)}"
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "guidance_scale": (
                    "FLOAT",
                    {"default": 3.5, "min": 1.0, "max": 32.0, "step": 0.05, "tooltip": "guidance scale"},
                ),
                "seed": ("INT", {"default": 42, "min": 0, "max": 0xFFFFFFFFFFFFFFFF, "step": 1}),
                "shift": (
                    "BOOLEAN",
                    {"default": True, "tooltip": "shift the schedule to favor high timesteps for higher signal images"},
                ),
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "width": (
                    "INT",
                    {"default": 768, "min": 256, "max": 4096, "step": 2, "tooltip": "width of the plot in pixels"},
                ),
                "height": (
                    "INT",
                    {"default": 512, "min": 256, "max": 4096, "step": 2, "tooltip": "height of the plot in pixels"},
                ),
...
class UploadToHuggingFace:
        ...
        return {
            ...
            "optional": {
                "token": (
                    ...
                    {
                        "default": "",
                        "tooltip": "DO NOT LEAVE IN THE NODE or it might save in metadata, can also use the hf_token.json",
                    },
                ),
            },
        }
    ...
    def upload(self, source_path, network_trainer, repo_id, private, revision, token=""):
        with torch.inference_mode(False):
            ...
            try:
                api.repo_info(repo_id=repo_id, revision=revision if revision != "" else None, repo_type=repo_type)
                repo_exists = True
            ...
            is_folder = (type(source_path) == str and os.path.isdir(source_path)) or (
                isinstance(source_path, Path) and source_path.is_dir()
            )
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "output_path": (
                    "STRING",
                    {"default": f"{str(os.path.join(folder_paths.models_dir, 'loras', 'Flux'))}"},
                ),
                "dim": ("INT", {"default": 4, "min": 2, "max": 1024, "step": 2, "tooltip": "LoRA rank"}),
                "save_dtype": (
                    ["fp32", "fp16", "bf16", "fp8_e4m3fn", "fp8_e5m2"],
                    {"default": "bf16", "tooltip": "the dtype to save the LoRA as"},
                ),
                ...
                "clamp_quantile": (
                    "FLOAT",
                    {"default": 0.99, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "clamp quantile"},
                ),
    ...
    def extract(
        ...
        outpath = svd(
            ...
            save_to=os.path.join(
                output_path,
                f"{finetuned_model.replace('.safetensors', '')}_extracted_lora_rank_{dim}-{save_dtype}.safetensors",
            ),
            dim=dim,
            device=load_device,
